...
def main(alert, send_sms_value, verbose):
    """Main function.

    :param alert: True for to check if there is a state for send an alert or false for get next trains.
    :type alert: bool

    :param send_sms_value: True for to send a SMS when there is a state or at each get next trains, depends of the alert state.
    :type send_sms_value: bool

    :param verbose: Print the return values.
    :type verbose: bool
    """
    if alert:
...
if __name__ == "__main__":
    ...
    main(alert, send_sms_value, verbose)
